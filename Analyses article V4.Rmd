...
#Impact of delay until the patients recieves its final diagnosis and the relapses


```

Among all patients the delay between the first episode and the final diagnosis was on average of `r as.numeric(delay_dx_all$MEAN)` days (SD = `r as.numeric(delay_dx_all$SD)`). As shown in Figure X and Table X, a Cox regression showed that epileptic patients (M= `r as.numeric(delay_dx_epi$MEAN)`; SD= `r delay_dx_epi$SD`) tend to reach their final diagnosis earlier than non-epileptic patients (M = `r as.numeric(delay_dx_non_epi$MEAN)`; SD = `r delay_dx_non_epi$SD`) (ASSUMPTIONS VIOLATED; HOW TO FIX IT ?).
Moreover, data show that patients who recieve their final diagnosis faster are less prone to relapse (STATS METHOD NEED TO BE DEFINED).
...
Relapse was defined for not epileptic patients as a new consultation in the ED or a written trace of a new event in a medical report of any specialities for similar motives as the ones of the first event. Regarding epileptic patients, only epileptic seizures were taken into account. Some patients after a first non provoked epileptic event sometimes presented an acute symptomatic seizure (N = `r N_acute_relapses`/`r nrow(data_full_cleaned%>%filter(dx.event == "Epilepsy"))`; `r pct_acute_relapses`). These event wer not retained as relapses. Among all patients, `r N_relapses_total` (`r pct.fun(N_relapses_total, nrow(data_full_cleaned))`) relapsed. The proportion is higher for epileptic patients (`r N_relapses_epi`/`r nrow(data_full_cleaned%>%filter(dx.event == "Epilepsy"))`; `r pct_relapses_epi`), compared to non epileptic patients (`r N_relapses_non_epi`/`r N_non_epi`; `r pct_relapses_non_epi`). This proportion was significantly different among the two groups (`r report_results(result_chi_epi_non_epi, method = "chisq")`). Among epileptic patients, focal epilepsy patients (`r N_relapses_focal`/`r N_focal`; `r pct_relapses_focal`)tend to relapse more than generalized epilepsy patients (`r N_relapses_generalized`/`r N_generalized`; `r pct_relapses_generalized`), however, this difference did not reached significance (`r report_results(res_chi_focal_gen, method = "chisq")`). Inside the focal epilepsy group, patient with non lesional epilepsy (`r N_relapses_non_les`/ `r N_non_les` ; `r pct_relapses_non_les`) are significantly most prone to relapse compared to lesional patients(`r N_relapses_les`/`r N_les`; `r pct_relapses_les`; `r report_results(res_chi_les_non_les, method = "chisq")`).
Regarding the non-epileptic groups, cardiac patients has the highest relapse rate compared to the other non-epileptic groups (`r N_relapses_cardiac`/`r N_cardiac`; `r pct_relapses_cardiac`), however, this difference did not reach significance either(`r report_results(res_chi_cardiac, "chisq")`). See Table X for a complete description of the relapse rate per diagnosis.
